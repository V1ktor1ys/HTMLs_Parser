...
# Load already parsed files (normalize paths to posix)
parsed_files = set()
if parsed_log_file.exists():
    with open(parsed_log_file, "r", encoding="utf-8") as f:
        parsed_files = {line.strip().replace("\\", "/") for line in f}
...
with open(output_file, "a", newline="", encoding="utf-8-sig") as csvfile:
    ...
    for folder in html_folders:
        ...
        # Determine max index expected in this folder (largest number found)
        max_idx = max((int(f.stem) for f in files if f.stem.isdigit()), default=0)

        for idx in range(1, max_idx + 1):
            target_file = folder / f"{idx}.html"
            if not target_file.exists():
                target_file = folder / f"{idx}.htm"

            date_str = folder.name.replace("htmls_", "")
            relative_path = target_file.relative_to(input_root).as_posix()

            if not target_file.exists():
                # Missing file → write empty row
            ...
            try:
                ...
                if site == "immoscout24":
                    ...
                    row_data = parse_immowelt(soup, target_file)
                else:
